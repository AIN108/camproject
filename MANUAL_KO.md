...
CamProject는 Python과 OpenCV를 이용하여 웹캠의 실시간 영상을 입력받고, 얼굴 검출 결과에 따라 카메라 영상과 동영상 파일을 전환해 출력하는 영상처리 실습 프로젝트입니다.
...
수업에서 소스 수정과 버전 비교를 함께 진행한다면 Git 사용을 권장합니다.
...
`VideoCapture(0)`과 `VideoCapture(1)`을 바꾸어 장치가 어떻게 선택되는지 확인합니다.
...
설정에 따라 검출과 오검출이 어떻게 변하는지 비교합니다.
...
값을 변경하여 중앙 검출 영역을 넓히거나 줄입니다.

### 실습 4 — 화면 복귀 시간 변경

```python
current_time - last_detected_time > 1
```

의 `1`을 변경하여 영상 전환 지연을 확인합니다.

### 실습 5 — 재생 영상 교체

`seoultech.mp4` 대신 직접 준비한 영상을 지정해 얼굴 검출 시 다른 콘텐츠가 재생되도록 수정합니다.

### 실습 6 — 영상 분할 방식 변경

`split_frame()` 함수를 분석하고 좌우 분할을 상하 분할 방식으로 바꿔봅니다.

---

## 24. 현재 버전의 제한사항

현재 CamProject는 완성형 CCTV 제품이 아니라 교육 및 실습용 소스 코드입니다.
...
따라서 수업 환경에 배포할 때는 학생 PC별 환경 차이를 고려해야 합니다.

---

## 25. README와 실제 코드의 관계
...
웹캠을 사용하는 수업에서는 촬영 대상자에게 카메라 사용 사실을 알리는 것이 좋습니다.
...
따라서 교육기관 내부 수업을 넘어 다음 용도로 사용하려는 경우에는 이용 조건을 확인하는 것이 좋습니다.
...
`onecam.py`는 OpenCV의 기본적인 카메라 및 얼굴 검출 흐름을 이해하는 데 적합합니다.

`twocam(turn_videotocctv).py`는 다중 카메라, 영상 상태 전환, 화면 분할과 다중 디스플레이 출력까지 확장하여 살펴볼 수 있는 예제입니다.
